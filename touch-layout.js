...
class ForestTouchLayout {
  constructor({onOpen,onClose,onReset}){
    this.onClose=onClose;this.onReset=onReset;this.editing=false;this.positions={};this.drag=null;
    ...
    this.settings.innerHTML='<button id="touch-layout-edit">⚙ POSITION DES BOUTONS</button><span id="touch-layout-help" hidden>Glissez chaque bouton vers la position souhaitée.</span><button id="touch-layout-reset" hidden>PAR DÉFAUT</button><button id="touch-layout-done" hidden>TERMINÉ</button>';
    panel.appendChild(this.settings);
    const get=id=>document.getElementById(id);
    get('touch-layout-edit').onclick=()=>{if(!onOpen())return;onReset();this.editing=true;panel.classList.add('controls-editing');this.setTools(true);};
  ...
  finish(){if(!this.editing)return;this.drag=null;this.editing=false;this.save();document.getElementById('game-panel').classList.remove('controls-editing');this.setTools(false);this.onReset();this.onClose();}
}
